...
inline void Erase(std::vector<int>& subject, std::vector<int>& eraser) {
...
inline double GetValue(Planet& p, int dist) {
	#define EPS 1.0e-10
	return pow(p.GrowthRate(), 2.0) / (p.NumShips() * dist + EPS);
}

inline int GetIncommingFleets(const int sid, std::vector<int>& FIDX, int remaining = 1000) {
	int numFleets = 0;
...
inline int GetStrength(const int tid, const int dist, std::vector<int>& PIDX, std::vector<int>& FIDX) {
	int strength = 0;
...
inline int GetHub(const int sid, const int tid) {
